"""
MINDCRAFT — Explorer Training Script (Curriculum PPO)
Trains Explorer character across 6 levels of procedural river & obstacle scenarios.
Features:
  - Vectorized environments
  - Generalized Advantage Estimation (GAE)
  - Intrinsic Potential Reward Shaping
...
def train_explorer(
    ...
    seed: int = 42,
    save_path: str = "models/checkpoints/explorer_v2.pt"
):
    ...
    print("========================================================================")
    print("  MINDCRAFT — TRAINING EXPLORER POLICY (CURRICULUM PPO)                 ")
    print(f"[*] Hardware Compute: {device} | Vector Envs: {num_envs} | Steps: {total_timesteps}")
    print("========================================================================")

    envs = [MindcraftWorldEnvironment(grid_size=32, character_role=MindcraftWorldEnvironment.ROLE_EXPLORER, curriculum_level=3) for _ in range(num_envs)]
    ...
    for update in range(1, num_updates + 1):
        # Progressively increase curriculum level
        curr_lvl = min(6, 1 + int((update / num_updates) * 5))
        for env in envs: env.curriculum_level = curr_lvl
        ...
        if update % 10 == 0 or update == num_updates:
            ...
            print(f"Update {update:3d}/{num_updates} | Lvl: {curr_lvl} | Mean Rew: {mean_rew:6.2f} | SPS: {sps}")

    torch.save(agent.state_dict(), save_path)
    print(f"[+] Explorer Policy successfully trained & saved to: {save_path}")

if __name__ == "__main__":
    train_explorer(total_timesteps=40000)
